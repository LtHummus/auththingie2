...
func TestFtueEnv_HandleFTUEStep0GET(t *testing.T) {
	...
	t.Run("basic case in docker", func(t *testing.T) {
		...
		assert.Contains(t, w.Body.String(), `<input type="text" name="domain" id="domain-field" required aria-label="Server Domain" value="example.com" autocomplete="off" autocorrect="off" spellcheck="false" />`)
		assert.Contains(t, w.Body.String(), `<input type="text" name="auth_url" id="auth-url-field" required aria-label="Auth URL Field" value="https://auth.example.com" autocomplete="off" autocorrect="off" spellcheck="false" />`)
	})
