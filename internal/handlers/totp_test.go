...
func TestEnv_HandleTOTPValidation(t *testing.T) {
	...
	t.Run("database error", func(t *testing.T) {
		_, db, ll, _, ruriv, _, e := makeTestEnv(t)
	...
	t.Run("IP is banned at outset", func(t *testing.T) {
		_, _, ll, _, ruriv, _, e := makeTestEnv(t)
	...
	t.Run("IP becomes banned after bad TOTP", func(t *testing.T) {
		...
		ll.On("MarkFailedAttempt", "totp_user_guid|test-user").Return(4, nil)
		db.On("GetUserByGuid", mock.Anything, "test-user").Return(&user.User{
			Id:       "test-user",
			Username: "testuser",
	...
	t.Run("user becomes locked after wrong TOTP", func(t *testing.T) {
		...
		ll.On("MarkFailedAttempt", "totp_user_guid|test-user").Return(0, loginlimit.ErrAccountLocked)
		db.On("GetUserByGuid", mock.Anything, "test-user").Return(&user.User{
			Id:       "test-user",
			Username: "testuser",
			TOTPSeed: &sampleTOTPSeed,
		...
		assert.Equal(t, http.StatusOK, w.Result().StatusCode)
		assert.Contains(t, w.Body.String(), "Incorrect TOTP Code. This account has been temporarily locked due to too many TOTP failures")
	})
...
func TestEnv_HandleTOTPSetup(t *testing.T) {
	...
	t.Run("enrollment data generated", func(t *testing.T) {
		...
		et, err := totp2.DecodeEnrollmentTicket(ticketMatches[1])
		assert.NoError(t, err)
	...
	t.Run("post -- expired ticket", func(t *testing.T) {
		...
		newTicket, err := totp2.DecodeEnrollmentTicket(ticketMatches[1])
		assert.NoError(t, err)
	...
	t.Run("post -- submitted code missing", func(t *testing.T) {
		...
		ticket, err := totp2.DecodeEnrollmentTicket(ticketMatches[1])
		assert.NoError(t, err)
