...
func makeMocks(t *testing.T) (*mocks.MockDB, *mocks.MockLoginLimiter, *viper.Viper, *ValidatorImpl) {
	...
	return mdb, mll, cfg, &ValidatorImpl{
		db:  mdb,
		ll:  mll,
		cfg: cfg,
	}
...
func TestValidatorImpl_Validate(t *testing.T) {
	defaults := viper.New()
	defaults.SetDefault(argon.MemoryKey, argon.DefaultMemory)
	defaults.SetDefault(argon.IterationKey, argon.DefaultIterations)
	defaults.SetDefault(argon.ParallelismKey, argon.DefaultParallelism)
	defaults.SetDefault(argon.SaltLengthKey, argon.DefaultSaltLength)
	defaults.SetDefault(argon.KeyLengthKey, argon.DefaultKeyLength)

	correctPassword := "P@ssw0rd!"
	correctPasswordHash, _ := argon.GenerateFromPassword(correctPassword, defaults)

	t.Run("ip is being blocked", func(t *testing.T) {
		_, mll, _, v := makeMocks(t)
	...
	t.Run("successful login -- needs migration", func(t *testing.T) {
		mdb, mll, cfg, v := makeMocks(t)

		mll.On("IsAccountLocked", "ip|127.0.0.1").Return(false)
		mll.On("IsAccountLocked", "username|username").Return(false)

		cfg.Set(argon.IterationKey, 1)
