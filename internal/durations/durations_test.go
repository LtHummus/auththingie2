...
func TestNiceDuration(t *testing.T) {
	...
	assert.Equal(t, "0 seconds", NiceDuration(1000*time.Microsecond))
	...
	assert.Equal(t, "1 second", NiceDuration(1500*time.Millisecond))
	...
	t.Run("test rounding down", func(t *testing.T) {
		...
		assert.Equal(t, "1 minute 59 seconds", NiceDuration(d))
	})
}
